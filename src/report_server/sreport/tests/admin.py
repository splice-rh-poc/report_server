...
class ImportAPITest(BaseReportTestCase):
    ...
    def create_JSON(self, instance_identifier='00:11',
                   product_pem='69',
                   cpu='1',
                   socket='1',
                   memory='604836'
    ...
    def test_post_202(self):
        resp = self.api_client.post(
            '/api/v1/productusage/', format='json', data=self.create_JSON())
        ...
        # print('number of items in quarantine db:' +
        # str(QuarantinedReportData.objects.all().count()))

        self.assertEqual(1, ReportData.objects.all(
        ).count(), 'product_usuage successfully imported')
        self.assertEqual(0, QuarantinedReportData.objects.all(
        ).count(), 'asserted no product usuage quarantined')
    ...
    def test_post_409(self):
        ...
        self.assertEqual(1, QuarantinedReportData.objects.all(
        ).count(), 'asserted 1 product usuage quarantined')

    def test_post_unique_ident(self):
        resp0 = self.api_client.post('/api/v1/productusage/', 
                                     format='json', 
                                     data=self.create_JSON(instance_identifier='00:11'))
        resp1 = self.api_client.post('/api/v1/productusage/',
                                     format='json', 
                                     data=self.create_JSON(instance_identifier='00:22'))
        self.assertEqual(
            202, resp0.status_code, 'http status code is expected')
        self.assertEqual(
            202, resp1.status_code, 'http status code is expected')

        self.assertEqual(2, ReportData.objects.all(
        ).count(), 'product_usuage successfully imported')
        self.assertEqual(0, QuarantinedReportData.objects.all(
        ).count(), 'asserted 1 product usuage quarantined')
