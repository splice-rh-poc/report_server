...
class Product_Def:
    ...
    def get_product_match(product, rhic, start, end, contract_number):
        ...
        if (product.name == "RHEL Server") or \
            ...
            count = RHEL_Server_High(product, rhic, start, end)
            if(count):
            ...
            count = RHEL_Server_Low(product, rhic, start, end)
            if(count):
                result_dict = build_result(product, rhic, start, end, contract_number, count)
                result_dict['facts'] = ' < 8GB  '
                count_list.append(result_dict)
            ...
            count = JBoss_EAP_VCPU_gt4(product, rhic, start, end)
            if(count):
                result_dict = build_result(product, rhic, start, end, contract_number, count)
                result_dict['facts'] = ' > 4 vCPU  '
                count_list.append(result_dict)
                
            count = JBoss_EAP_VCPU_lt4(product, rhic, start, end)
            if(count):
                result_dict = build_result(product, rhic, start, end, contract_number, count)
                result_dict['facts'] = ' < 4 vCPU  '
                count_list.append(result_dict)
            ...
            count = OpenShift_Gear_high(product, rhic, start, end)
            if(count):
                result_dict = build_result(product, rhic, start, end, contract_number, count)
                result_dict['facts'] = ' > 1 vCPU, > 2GB  '
                count_list.append(result_dict)
            
            count = OpenShift_Gear_low(product, rhic, start, end)
            if(count):
                result_dict = build_result(product, rhic, start, end, contract_number, count)
                result_dict['facts'] = '  1 vCPU, <= 2GB  '
                count_list.append(result_dict)
...
def RHEL_Server_High(product, rhic, start, end):
        mem_high = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, memtotal__gte=8388608, sla=product.sla, support=product.support_level).count()
...
def RHEL_Server_Low(product, rhic, start, end):
        mem_low = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, memtotal__lt=8388608, sla=product.sla, support=product.support_level).count()
                            
        return mem_low
    
    
def JBoss_EAP_VCPU_gt4(product, rhic, start, end):
        jboss_high = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, cpu_sockets__gte=4, sla=product.sla, support=product.support_level).count()
...
def JBoss_EAP_VCPU_lt4(product, rhic, start, end):
        jboss_high = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, cpu_sockets__lt=4, sla=product.sla, support=product.support_level).count()
                            
        return jboss_high
    
def OpenShift_Gear_high(product, rhic, start, end):
        high = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, cpu_sockets__gte=4, memtotal__gte=8388608, sla=product.sla, support=product.support_level).count()
                            
        return high
    
def OpenShift_Gear_low(product, rhic, start, end):
        low = ReportData.objects.filter(consumer_uuid=str(rhic.uuid), \
          product=str(product.engineering_ids), date__gt=start, \
          date__lt=end, cpu_sockets__lt=4, memtotal__lt=8388608, sla=product.sla, support=product.support_level).count()
